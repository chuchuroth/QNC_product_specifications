...
below is a **companion Interface Control Document (ICD)** written in the same formal, production-oriented style as the rewritten PRD/SDS. This version is intentionally scoped to be consistent, implementable, and free of the major ambiguities identified in the original QNC materials, especially around product boundary, supported protocols, device semantics, and safety ownership. The baseline product context and original intent come from the uploaded QNC source document [Source](https://www.genspark.ai/api/files/s/MQZoL9fC).
