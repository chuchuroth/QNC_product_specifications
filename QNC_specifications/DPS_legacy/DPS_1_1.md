...
|---------|------------|----------------------|----------------|
| 1.0     | 2026-03-25 | Systems Engineering  | Initial companion Device Profile Specification aligned with QNC PRS/ICD baselines |
| 1.1     | 2026-04    | Systems Engineering  | Aligned structure, terminology, and release-scope model with QNC-PRD-001 v2.3; baseline and extension southbound protocols; DDS artifact boundary; Safe Mode and security cross-references; removed external reference links |
...
22. Relationship to DDS and Northbound Mapping Artifacts  
...
This **Device Profile Specification (DPS)** defines the **structure, content, validation rules, compatibility rules, lifecycle rules, and governance requirements** for **QNC (Quick Network Connector) device profiles**.

A **device profile** is a **versioned, validated, controlled artifact** (PRS Section 7.2) that allows QNC to onboard a **supported** industrial end-effector or field device within an **approved southbound protocol** and **device-class** boundary—using declarative mappings rather than ad hoc, ungoverned device-specific behavior.
...
- how profiles define supported commands, parameters, status, telemetry, and faults,
- how profiles map **southbound** device protocol semantics into QNC normalized abstractions,
- how profiles are validated, versioned, approved, deployed, and retired.

## 1.3 Out of Scope

This document does **not** define:
- product purpose, scope tiers, non-goals, or roadmap (governed by **QNC-PRD-001**),
- northbound API transport contracts (governed by the **ICD** and OpenAPI / WebSocket / logging artifacts),
- **DDS topic IDL schemas**, **DDS QoS profiles**, or **DDS-to-northbound mapping rules** (governed by PRS Sections 12.1, 13.3, and the **QNC FastDDS IDL Type Registry**; see Section 22),
- bit-exact protocol frame syntax for every industrial protocol (may reside in protocol conformance / module specifications),
- hardware connector drawings or manufacturing data,
- certified **functional safety** design or safety case evidence,
- semantics for devices outside an **approved, released** profile.

Companion specifications: **PRS** (boundary), **ICD** (interfaces), **DPS** (this document, device profile artifacts).
...
- **QNC Interface Control Document (ICD), QNC-ICD-001, v2.3** (or current released ICD revision)
- **QNC Protocol & Service Module Specification** (to be issued), including southbound stack and FastDDS module behavior where applicable
- **QNC FastDDS IDL Type Registry** (to be issued), for approved DDS topics and compatibility matrices when the FastDDS extension is released
- **QNC Verification & Validation Plan** (to be issued)
- **QNC Operations & Recovery Guide** (to be issued)

Where conflict exists:
1. **QNC-PRD-001** governs product definition, **baseline / approved extension / advanced gateway** scope, architecture principles, functional and non-functional requirements, safety position, security requirements, and configuration governance.
2. The **ICD** governs external interface contracts, operational modes, and interface-level fault and state behavior.
3. This **DPS** governs **device profile** artifact structure and profile-local semantics only.

Device profiles SHALL remain consistent with the PRS principle of **unified integration, not native universality**: no profile SHALL imply support for protocols or device classes outside **released** QNC scope.
...
- **southbound, device-specific protocol details**, and
- QNC **Profile & Semantic Normalization Layer** abstractions (PRS Section 7.1), as exposed through the **Integration & Control Layer** and **Northbound Edge Services Layer** via approved interfaces.

The profile SHALL allow QNC to expose **approved device-class** semantics without claiming **universal** semantics across unrelated devices or device classes. Device-aware mapping SHALL be explicit, versioned, and validation-gated in the profile artifact.

## 4.3 Architectural Boundary

A profile SHALL:
- describe and constrain **supported** device behavior within a declared protocol binding and release scope,
- define how QNC interacts with the device on the **southbound** link,
- define mappings into QNC normalized command, telemetry, status, and fault structures.

A profile SHALL NOT:
- redefine QNC system-level **lifecycle** or **Safe Mode (Operational Inhibit Mode)** definitions (PRS Section 6; ICD),
- change **northbound** or robot-facing interface contracts (ICD, OpenAPI, WebSocket schemas),
- introduce **southbound** protocol families or bindings outside the **released** scope for the target firmware build,
- redefine **functional safety** ownership (PRS Section 10),
- claim **Released** status without completing validation and approval per PRS Section 5.5.2 and this DPS.
...
This prevents the profile from becoming an uncontrolled scripting mechanism and supports **controlled extensibility** (PRS Sections 4.1, 7.3).
...
Every profile SHALL declare exactly one **device class**. Baseline illustrative classes (non-exclusive; the authoritative class catalog SHALL be project-controlled) include:

- `GRIPPER_ELECTRIC`
- `ACTUATOR_SIMPLE`
- `SENSOR_INDUSTRIAL`

Additional or renamed classes require formal governance per **PRS Section 18** and validation updates.
...
## 6.4 Release Scope Alignment

Each profile SHALL be authored for a defined **QNC release scope** (PRS Section 5):

- **Baseline** profiles use only **baseline** southbound protocol bindings (PRS Section 5.2).
- **Extension** profiles MAY use **approved extension** southbound bindings (PRS Section 5.3) only when those protocols are **released** in the target firmware and validation evidence exists.
- Profiles SHALL NOT claim bindings to **advanced gateway direction** protocols (e.g., EtherCAT, PROFINET) until those capabilities enter an approved release with applicable hardware and module specifications.

The `protocol_binding` and `compatibility` sections SHALL make the intended release scope explicit so that QNC can **refuse activation** of profiles outside the installed build’s released capabilities (PRS Sections 5.1, CFG-003).
...
## 9.1 Protocol Declaration and Identifiers

Each profile SHALL declare exactly one **southbound** protocol binding. The binding identifier SHALL be chosen from the set **released** for the target QNC build.

**Baseline** southbound protocol identifiers (PRS Section 5.2), when supported by the runtime, include:

| Identifier | Description |
|------------|-------------|
| `IO_LINK` | IO-Link v1.1 master semantics |
| `MODBUS_RTU` | Modbus RTU (serial) |
| `ETHERNET_IP_ADAPTER` | EtherNet/IP adapter mode, Class 1 and Class 3 |
| `DISCRETE_DIGITAL_IO` | Discrete digital I/O (sourcing/sinking configurable) |

**Approved extension** southbound identifiers (PRS Section 5.3), when released for a build, include non-exclusively:

| Identifier | Description |
|------------|-------------|
| `MODBUS_TCP` | Modbus TCP |
| `CANOPEN` | CANopen (CiA 301/302 device support as released) |
| `SERIAL_CAN_APPROVED` | Placeholder for additional **formally approved** serial/CAN-based protocols |

**Advanced gateway direction** protocols (e.g., EtherCAT, PROFINET) SHALL NOT appear in a profile until they are **released** with validated hardware and module specifications (PRS Section 5.4).

Protocol-specific fields within `protocol_binding` (e.g., CIP connection parameters, IO-Link port mapping, coil/register addressing) SHALL be documented in the **QNC Protocol & Service Module Specification** or companion binding guides as needed.
...
A profile SHALL NOT:
- declare protocol families outside the **released** scope for the target QNC build (PRS Section 5),
- assume protocol behavior not represented in the **ICD** or **Protocol & Service Module Specification**,
- require runtime scripting to complete normal protocol operations,
- imply **universal** interoperability across vendors without validated identity and mapping rules.
...
| `safety_behavior` | object | No | Restrictions under **Safe Mode** or degraded modes (operational inhibit / fault policy; not functional safety) |
...
| `source_layer` | enum | Yes | Protocol / device / profile (and `dds` when DDS-derived faults are mapped, if applicable) |
| `summary` | string | Yes | Human-readable summary |
| `recovery_guidance` | string | No | Recommended action |
| `safe_mode_required` | boolean | Yes | Whether mapped fault SHALL trigger **Safe Mode (Operational Inhibit Mode)** per QNC fault policy (PRS FR-062; ICD) |
...
QNC is **not a safety-rated system** (PRS Section 10). Device conditions that require **functional safety** response SHALL be outside QNC’s safety authority.

If a profile identifies a device condition that warrants **operational inhibit** or **Safe Mode** entry, it SHALL:
- mark it explicitly in the fault mapping,
- define expected QNC **local** response consistent with ICD fault and mode behavior (e.g., actuation inhibit, structured fault publication),
- **not** redefine system-level **safety controller** ownership or certified safety functions.
...
| `on_failure` | enum | Yes | Abort / Retry / Degrade / SafeMode (Safe Mode = Operational Inhibit Mode) |
...
- required interface schema version (ICD / OpenAPI / message schema revision as applicable),
- protocol adapter version constraints,
- **released southbound protocol scope** or build identifier the profile was validated against (SHALL align with PRS Section 5),
- supported device revisions,
- incompatibility notes.

When a deployment uses the **FastDDS extension**, DDS-facing compatibility MAY additionally reference the **Robot Software Compatibility Matrix** (PRS Section 13.3); that matrix is **not** a substitute for device profile fields for **southbound** devices.
...
- schema validation passed (PRS **CFG-001**, **CFG-003**),
- integration and protocol conformance tests passed for the declared **southbound** binding,
- fault handling and **Safe Mode** interaction tests passed as required by the **QNC Verification & Validation Plan** (PRS Section 15),
- compatibility checks passed,
- required stakeholders approved per project governance (PRS Section 18).
...
Requirements SHALL be consistent with **PRS Section 11** (e.g., **SEC-006** secure update and integrity expectations for controlled artifacts).
...
- retain the last known valid active configuration where possible (PRS FR-020, CFG-004).
...
- protocol binding uses **released** identifiers for the target QNC build (Section 9),
...
Conformance to this specification does not by itself imply **baseline** or **extension** release support; it establishes that the artifact is structurally valid and governable. **Released** status requires completion of the approval and validation path in Section 17 and PRS **Section 5.5.2**.

---

# 22. Relationship to DDS and Northbound Mapping Artifacts

**Device profiles** governed by this DPS apply to **southbound** industrial end-effectors and field devices.

**DDS / ROS 2** integration artifacts are **distinct** configuration types under PRS Section 12.1:

- **DDS IDL schemas** — versioned IDL types for approved topic subscriptions and publications.
- **DDS QoS profiles** — QoS policy definitions per topic and deployment mode.
- **IDL Type Registry** and **QoS Profile Catalog** — governed catalogs (PRS Section 13.3).

Those artifacts SHALL be maintained in the **QNC FastDDS IDL Type Registry** and related companion specifications, **not** as substitutes for the YAML sections defined in Section 7 of this DPS.

When **DDS-to-northbound bridging** is released (PRS Section 5.3), mapping from DDS-derived data to OPC UA, MQTT, or other northbound schemas SHALL be validated and documented per PRS **FR-054** and the ICD; such mappings MAY reference internal normalization concepts that align with profile semantics but SHALL not bypass IDL and QoS governance.

---

# Appendix A. Normative YAML Template

The following template is **illustrative** of structure and uses a **Modbus RTU** binding. Profiles for **IO-Link**, **EtherNet/IP**, or **discrete I/O** SHALL use the same top-level sections with `protocol_binding` fields appropriate to those stacks, as defined in the **QNC Protocol & Service Module Specification** and validated for the target release.
...
| Protocol binding supported and within **released** QNC scope (PRS Section 5) |
...
## D.6 Southbound protocol identifiers
Values of `protocol_binding.protocol` SHALL match **released** identifiers from Section 9.1 (e.g., `MODBUS_RTU`, `IO_LINK`, `ETHERNET_IP_ADAPTER`, `DISCRETE_DIGITAL_IO`, and extension identifiers when released).

---

## Document Positioning

This **Device Profile Specification** is the normative definition of **QNC device profile** artifacts. Together with **QNC-PRD-001** (strategy and scope), the **ICD** (interfaces), and the **FastDDS IDL Type Registry** (DDS artifacts where released), it supports **profile-based device governance** (PRS Section 7.2) and **controlled extensibility** without overclaiming universal protocol or device support. Changes to profile schema or governance rules SHALL follow **PRS Section 18**.
