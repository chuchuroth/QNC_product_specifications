...
> The goal is not just to run on Qualcomm — it is to **offload core AI and perception workloads to NPU(AI) and DSP(传感器融合)**, while keeping our existing Docker/ROS 2 stack hardware‑agnostic.  
...
|------------|----------------|
| **NPU + DSP + ISP(视觉处理) integration** | Enables <15ms local inference without a separate GPU |
